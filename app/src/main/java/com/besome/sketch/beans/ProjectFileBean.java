...
public class ProjectFileBean extends SelectableBean implements Parcelable {
	...
	public String getDrawersJavaName() {
		String str = "";
		if (this.fileType == THEME_FULLSCREEN && this.fileName.indexOf("_drawer_") >= 0) {
			return getJavaName(this.fileName.substring(OPTION_ACTIVITY_FAB));
		}
		return str;
	}
	...
	public static String getActivityNameOld(String str) {
		...
		while (i < str.length()) {
			...
			if (charAt == '_' && i < str.length() + THEME_NONE) {
				int i2 = i + THEME_NOACTIONBAR;
				char charAt2 = str.charAt(i2);
				if (Character.isLowerCase(charAt2)) {
					StringBuilder stringBuilder2 = new StringBuilder();
					stringBuilder2.append(str2);
					stringBuilder2.append(Character.toUpperCase(charAt2));
